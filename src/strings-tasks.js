...
function removeFirstOccurrences(str, value) {
  const occurrence = str.indexOf(value);
  if (occurrence === -1) return str;

  return (
    str.substring(0, occurrence) + str.substring(occurrence + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const occurrence = str.lastIndexOf(value);
...
function isPalindrome(str) {
  const normStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function extractNameFromTemplate(value) {
  return removeLastOccurrences(value.substring(value.indexOf(',') + 2), '!');
}
...
function unbracketTag(str) {
  ...
  result = removeFirstOccurrences(result, '>');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < characters.length; i += 1) {
    if (/[a-zA-Z]/.test(characters[i])) {
      const base = characters[i] <= 'Z' ? 'A' : 'a';
      characters[i] = String.fromCharCode(
        ((characters[i].charCodeAt(0) - base.charCodeAt(0) + 13) % 26) +
          base.charCodeAt(0)
      );
    }
  }
...
function getCardId(value) {
  // Define the suits and the ranks
  const suits = ['♣', '♦', '♥', '♠'];
  const ranks = [
    'A',
    '2',
